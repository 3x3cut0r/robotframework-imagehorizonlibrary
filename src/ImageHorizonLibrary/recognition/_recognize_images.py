...
class _RecognizeImages(object):
    ...
    def click_image(self, reference_image, timeout=dflt_timeout):
        ...
        LOGGER.info(
            'Clicking image "%s" in position %s' % (reference_image, center_location)
        )
    ...
    def _locate_and_click_direction(
        ...
        self._click_to_the_direction_of(
            direction, location, offset, clicks, button, interval
        )
    ...
    def _locate(self, reference_image, log_it=True):
        ...
        if log_it:
            LOGGER.info(
                'Image "%s" found at %r (strategy: %s)'
                % (reference_image, (x, y), self.strategy)
            )
        return (x, y)
    ...
    def _try_locate(self, ref_image, haystack_image=None, locate_all=False):
        ...
        with ih._suppress_keyword_on_failure():
            ...
            ih.peakmap = match_template(
                ih.haystack_edge, ih.needle_edge, pad_input=True
            )
            ...
            if locate_all:
                ...
                for i, pk in enumerate(peak_coords):
                    ...
                    if peak > confidence:
                        loc = (
                            x - needle_img_width / 2,
                            y - needle_img_height / 2,
                            needle_img_width,
                            needle_img_height,
                        )
                        location.append(loc)
